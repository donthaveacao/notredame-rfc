...
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
// Hi ~Wei
double claw_servo_position;
double arm_servo_velocity;
double turntable_servo_position;
Servo claw_servo;
Servo arm_servo;
...
void setup() {
  ...
  arm_servo.write(arm_servo_velocity);
  // Bye ~Wei
  // XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
if(rxdata.get(BACK)){
  // retract (continuous) arm (set velocity to -X) 
  arm_servo_velocity = ARM_ZERO+ARM_SPEED;  
}
else if(rxdata.get(START)){
  // extend arm (set velocity to +X)
  arm_servo_velocity = ARM_ZERO-ARM_SPEED;
}
else{
...
// TURNTABLE ROTATION CODE (POSITION)
/*
Uses variables:
turntable_servo
turntable_servo_velocity
TURNTABLE_SPEED
TURNTABLE_UPPER_LIMIT
TURNTABLE_LOWER_LIMIT
*/
...
if(rxdata.ButtonPress(A)){
  ...
  if(turntable_servo_position>TURNTABLE_LOWER_LIMIT){
    ...
    turntable_servo_position = TURNTABLE_LOWER_LIMIT;
  }
}
turntable_servo.write(turntable_servo_position);
...
if(rxdata.ButtonPress(X)){
  ...
  claw_servo_position -= CLAW_SPEED;
    ...
    claw_servo_position = CLAW_LOWER_LIMIT;
  }
}
claw_servo.write(claw_servo_position);
